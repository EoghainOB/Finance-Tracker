...
async function main() {
  ...
  await connect (`mongodb+srv://Eoghain:${process.env.REACT_APP_PASS}@cluster0.sdqzzhz.mongodb.net/finance?retryWrites=true&w=majority`);
  ...
  app.get("/api/expenses", async (_req, res) => {
    try {
    const allCarts = await Expenses.find();
    res.json(allCarts);
  } catch (err) {
    res
    .status(400)
  ...
  app.put('/api/expenses/:id', async (req, res) => {
    try {
    const { id } = req.params;
    const updatedExpense = await Expenses.findByIdAndUpdate(id, { ...req.body })
    res
      .status(201)
      .json(updatedExpense);
    }
